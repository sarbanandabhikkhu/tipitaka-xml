const webFXTreeConfig = {
  rootIcon: "./assets/svg/folderIcon.svg",
  openRootIcon: "./assets/svg/openFolderIcon.svg",
  folderIcon: "./assets/svg/folderIcon.svg",
  openFolderIcon: "./assets/svg/openFolderIcon.svg",
  fileIcon: "./assets/svg/fileIcon.svg",
  iIcon: "./assets/svg/iIcon.svg",
  lIcon: "./assets/svg/lIcon.svg",
  lMinusIcon: "./assets/svg/lMinusIcon.svg",
  lPlusIcon: "./assets/svg/lPlusIcon.svg",
  tIcon: "./assets/svg/tIcon.svg",
  tMinusIcon: "./assets/svg/tMinusIcon.svg",
  tPlusIcon: "./assets/svg/tPlusIcon.svg",
  blankIcon: "./assets/svg/blankIcon.svg",
...
function WebFXTree(e, t, o, i, n) {
  (this.base = WebFXTreeAbstractNode),
    this.base(e, t),
    (this.icon = i || webFXTreeConfig.rootIcon),
    (this.openIcon = n || webFXTreeConfig.openRootIcon),
    webFXTreeConfig.usePersistence
      ? (this.open =
          "0" !=
          webFXTreeHandler.cookies.getCookie(
...
function _xmlTreeToJsTree(e) {
  ...
  (t =
    ...
    "" != d && (t.target = d);
  for (let l = e.childNodes, c = l.length, h = 0; h < c; h++)
...
function _xmlFileLoaded(e, t) {
  if (!t.loaded) {
    ...
    if (
      ((t.loaded = !0),
      (t.loading = !1),
      null == e || null == e.documentElement)
    )
      alert(e.xml),
        (t.errorText = parseTemplateString(
          webFXTreeConfig.loadErrorTextTemplate,
          t.src
        ));
    else {
      for (
        let n = e.documentElement.childNodes, r = n.length, s = 0;
        s < r;
        s++
      )
        "tree" == n[s].tagName &&
...
function parseTemplateString(e) {
  ...
  for (let i = 1; i < t.length; i++)
    o = o.replace(new RegExp("%" + i + "%", "g"), t[i]);
  return o;
}
...
(WebFXCookie.prototype.setCookie = function (e, t) {
  ...
  (WebFXTreeAbstractNode.prototype.add = function (e, t) {
    (e.parentNode = this), (this.childNodes[this.childNodes.length] = e);
    let o = this;
    for (
      this.childNodes.length >= 2 &&
      (this.childNodes[this.childNodes.length - 2]._last = !1);
      o.parentNode;
    ...
    if (o.rendered) {
      this.childNodes.length >= 2 &&
        ((document.getElementById(
          this.childNodes[this.childNodes.length - 2].id + "-plus"
        ).src = this.childNodes[this.childNodes.length - 2].folder
          ? this.childNodes[this.childNodes.length - 2].open
            ? webFXTreeConfig.tMinusIcon
            : webFXTreeConfig.tPlusIcon
          : webFXTreeConfig.tIcon),
        (this.childNodes[this.childNodes.length - 2].plusIcon =
          webFXTreeConfig.tPlusIcon),
        (this.childNodes[this.childNodes.length - 2].minusIcon =
          webFXTreeConfig.tMinusIcon),
        (this.childNodes[this.childNodes.length - 2]._last = !1)),
        (this._last = !0);
      for (let i = this; i.parentNode; ) {
        ...
        for (
          n = 0;
          n < i.parentNode.childNodes.length &&
          i.id != i.parentNode.childNodes[n].id;
          n++
        );
        n == i.parentNode.childNodes.length - 1
          ? (i.parentNode._last = !0)
          : (i.parentNode._last = !1),
          (i = i.parentNode);
      }
  ...
  (WebFXTreeAbstractNode.prototype.indent = function (e, t, o, i, n) {
    ...
    return (
      s &&
        (s._last || (t && o)
          ? (s.src = webFXTreeConfig.blankIcon)
          : (s.src = webFXTreeConfig.iIcon)),
      0
    );
  }),
  ...
  (WebFXTreeItem.prototype.remove = function () {
    var e = document.getElementById(this.id + "-plus").src,
      ...
      n =
        ...
        !i || !i.parentNode || i.parentNode.id != t.id);
    this.getPreviousSibling().focus(),
      ...
      o != t ||
        ...
        (document.getElementById(o.id + "-icon").src =
      ...
      (o = o.parentNode),
        (i =
          `<img id="${this.id}-indent-${n}" src="${
            o._last ? webFXTreeConfig.blankIcon : webFXTreeConfig.iIcon
          }">` + i),
        n++;
    (this._level = n),
      this.childNodes.length ? (this.folder = 1) : (this.open = !1),
      this.folder || "classic" != webFXTreeHandler.behavior
        ? (this.icon || (this.icon = webFXTreeConfig.folderIcon),
          this.openIcon || (this.openIcon = webFXTreeConfig.openFolderIcon))
  ...
  (webFXTreeConfig.emptyErrorTextTemplate =
    'Error "%1%" does not contain any tree items'),
